...
var temporaryFileLineLimits = map[string]packageLimit{
	...
	"packages/coding-agent/core/interactive_tui.go": {MaxLines: 1460, Reason: "[P2-02 TUI rich-UX: input history ring (addToHistory/navigateHistory + Up/Down browsing) and @ file-reference autocomplete (fileReferenceSuggestions incl. absolute/quoted-space paths via unclosedAtQuoteStart + token-aware Tab); TEMPORARY — to be relocated by the planned core/modes subpackage split] ratcheted for P1-4 per-command child-context cancellation (beginCommand/clearCommandCancel + Escape handling so a running slash/bash command can be aborted, not just an agent turn) + interactive-TUI slice 1 Ctrl+P/Shift+Ctrl+P model cycling (key cases + cycleModel handler) + interactive-TUI slice 2 /model selector overlay wiring (modelSelector field, key routing, Ctrl+L + bare-/model entry, View input-region swap, openModelSelector/handleModelSelectorKey/applyModelSelection); overlay struct/render/key logic lives in interactive_model_selector.go; awaits further selector/overlay extraction"},
}
