...
var temporaryPackageLimits = map[string]packageLimit{
	...
	"packages/ai":                      {MaxFiles: 34, MaxLines: 8455, Reason: "post-parity-fix ratcheted budget; includes existing credential-dir chmod tightening; remaining oauth/model catalog/root provider adapters, programmable faux provider, retry classification (IsRetryableProviderError), strict cross-provider handoff same-model check (transform_messages), and P2 wrap-up (OAuth identity cache_control, validation casing, azure prompt_cache_key gate, cloudflare punctuation, 2-space auth.json) plus session summary text compatibility await final package split; + review-parity round: stream finish_reason truncation guard, Bedrock reserved-header skip, OpenRouter developer-role + tool-result name"},
	"packages/ai/providers":            {MaxFiles: 15, MaxLines: 5518, Reason: "post-parity-fix ratcheted budget; provider protocol implementations migrated from packages/ai root; split by provider family if this grows further; + review-parity round: SetEscapeHTML(false) MarshalJSON/UnescapeJSONHTML + streaming cache-write/DeepSeek-cache-hit usage"},
	...
	"packages/coding-agent/core":       {MaxFiles: 28, MaxLines: 12028, Reason: "post-parity-fix ratcheted P0/P1 session/runtime parity budget plus session-info parity, uncapped JSONL line reader (P0-4), ThinkingBudgets getter + SessionID/Transport/ThinkingBudgets/MaxRetryDelayMs AgentOptions wiring (P1-A1), print-mode text output parity and exit-code cleanup (P1-A3: final-assistant-text-only on stdout, error/aborted to stderr + exit 1), BinDir() + bash shell-env PATH injection wiring (P1-G1), P2 context-file discovery parity (4 casings, first-per-dir, global->root->cwd order, no re-sort), P1-5 interactive cross-project fork + missing-cwd continue prompts (cli.Confirm) with session cwdOverride, P2-1 system-prompt project-context/skills XML shapes, and extension tool_result mutated-input replay; awaits runtime/session/modes subpackage split; + review-parity round: default model/thinking persistence, enabledModels cycle scope, RPC bash sanitize/truncate + RPC SetEscapeHTML(false), ext ui.confirm/select errors + register* graceful-degrade, pi-update npm reinstall"},
