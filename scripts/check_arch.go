...
var temporaryPackageLimits = map[string]packageLimit{
	"packages/agent/harness":           {MaxFiles: 16, MaxLines: 3752, Reason: "post-parity-fix ratcheted budget; includes P1-C1 emitRunFailure run-failure termination sequence in harness.go; awaits full prompt/session/hook subpackage extraction"},
	"packages/ai":                      {MaxFiles: 34, MaxLines: 8316, Reason: "post-parity-fix ratcheted budget; includes existing credential-dir chmod tightening; remaining oauth/model catalog/root provider adapters, programmable faux provider, retry classification (IsRetryableProviderError), strict cross-provider handoff same-model check (transform_messages), and P2 wrap-up (OAuth identity cache_control, validation casing, azure prompt_cache_key gate, cloudflare punctuation, 2-space auth.json) plus session summary text compatibility await final package split"},
	"packages/ai/providers":            {MaxFiles: 15, MaxLines: 5360, Reason: "post-parity-fix ratcheted budget; provider protocol implementations migrated from packages/ai root; split by provider family if this grows further"},
	...
	"packages/coding-agent/core":       {MaxFiles: 28, MaxLines: 11939, Reason: "post-parity-fix ratcheted P0/P1 session/runtime parity budget plus session-info parity, uncapped JSONL line reader (P0-4), ThinkingBudgets getter + SessionID/Transport/ThinkingBudgets/MaxRetryDelayMs AgentOptions wiring (P1-A1), print-mode text output parity (P1-A3: final-assistant-text-only on stdout, error/aborted to stderr + exit 1), BinDir() + bash shell-env PATH injection wiring (P1-G1), P2 context-file discovery parity (4 casings, first-per-dir, global->root->cwd order, no re-sort), P1-5 interactive cross-project fork + missing-cwd continue prompts (cli.Confirm) with session cwdOverride, P2-1 system-prompt project-context/skills XML shapes, and extension tool_result mutated-input replay; awaits runtime/session/modes subpackage split"},
	"packages/coding-agent/core/tools": {MaxFiles: 25, MaxLines: 2774, Reason: "ratcheted budget; one file per tool plus platform-split exec/replace files (bash_exec_unix/windows/other, file_replace_*), edit fuzzy NFKC/error-message + read/ls wording parity helpers, ShellEnv() PATH-injection helper (P1-G1), shared FileURLToPath parser (fileurl.go) reused by tools/path.go, NormalizePath, and the CLI file processor (P2-3 file:// unification), ripgrep-preferring grep with RE2 fallback (P2-2), and the detached-child PID registry/process-group liveness checks (detached.go + killProcessTreeByPID, P2-3); split if tool bodies grow"},
...
var temporaryFileLineLimits = map[string]packageLimit{
	...
	"packages/coding-agent/core/interactive_tui.go": {MaxLines: 1021, Reason: "ratcheted for P1-4 per-command child-context cancellation (beginCommand/clearCommandCancel + Escape handling so a running slash/bash command can be aborted, not just an agent turn); awaits interactive selector/overlay extraction"},
}
