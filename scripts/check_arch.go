...
var temporaryPackageLimits = map[string]packageLimit{
	...
	"packages/coding-agent/core":       {MaxFiles: 28, MaxLines: 12100, Reason: "post-parity-fix ratcheted P0/P1 session/runtime parity budget plus session-info parity, uncapped JSONL line reader (P0-4), ThinkingBudgets getter + SessionID/Transport/ThinkingBudgets/MaxRetryDelayMs AgentOptions wiring (P1-A1), print-mode text output parity and exit-code cleanup (P1-A3: final-assistant-text-only on stdout, error/aborted to stderr + exit 1), BinDir() + bash shell-env PATH injection wiring (P1-G1), P2 context-file discovery parity (4 casings, first-per-dir, global->root->cwd order, no re-sort), P1-5 interactive cross-project fork + missing-cwd continue prompts (cli.Confirm) with session cwdOverride, P2-1 system-prompt project-context/skills XML shapes, and extension tool_result mutated-input replay; awaits runtime/session/modes subpackage split; + review-parity round: default model/thinking persistence, enabledModels cycle scope, RPC bash sanitize/truncate + RPC SetEscapeHTML(false), ext ui.confirm/select errors + register* graceful-degrade, pi-update npm reinstall; + interactive-TUI slice 1: Ctrl+P/Shift+Ctrl+P model cycling (CycleModelBackward + cycleModel handler)"},
...
var temporaryFileLineLimits = map[string]packageLimit{
	...
	"packages/coding-agent/core/interactive_tui.go": {MaxLines: 1077, Reason: "ratcheted for P1-4 per-command child-context cancellation (beginCommand/clearCommandCancel + Escape handling so a running slash/bash command can be aborted, not just an agent turn) + interactive-TUI slice 1 Ctrl+P/Shift+Ctrl+P model cycling (key cases + cycleModel handler); awaits interactive selector/overlay extraction (slice 2)"},
}
