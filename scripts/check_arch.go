...
var temporaryFileLineLimits = map[string]packageLimit{
	...
	"packages/coding-agent/core/modes.go":                     {MaxLines: 1137, Reason: "[catalog-fix round: /login slashPrompter wiring + /debug command handling so OAuth login completes inside the interactive TUI] + remaining-parity workstream 2 ExtensionContext mode binding + remaining-parity workstream 5 /hotkeys extension shortcut listing + extension UI setStatus parser/no-op line-mode bridge"},
	"packages/coding-agent/core/interactive_tui.go":           {MaxLines: 1366, Reason: "[god-file split] reduced 2963->1366 by extracting interactive_autocomplete.go (~798, completion/suggestion code) and interactive_extension_ui.go (~825, ctx.ui handler/state/widget/custom-message code), both under the default. Retains the interactiveModel struct, Update event loop, View, model lifecycle, key handling, command running, and agent/session event handling. Awaits a further input/command-running extraction if it grows."},
	"packages/coding-agent/core/session.go":                   {MaxLines: 1037, Reason: "session JSONL/session-manager budget includes TS-parity HasEntry lookup for extension setLabel validation; split session helpers after parity work stabilizes; + review-fix: EntriesSnapshot/CurrentLeafID locked accessors so extension/RPC readers no longer race Append on Entries/CurrentID"},
	"packages/coding-agent/core/extensions/script_runtime_bridge.go": {MaxLines: 1219, Reason: "[god-file split] the embedded Node-bridge source (scriptRuntimeBridge const) extracted verbatim from script_runtime.go into its own file: a single cohesive data blob, not logic. script_runtime.go itself is now ~618 lines (under the default) and needs no special budget; script_runtime_ipc.go (~415, the readLoop/request/ProviderStream transport) is also under the default."},
}
