...
var temporaryPackageLimits = map[string]packageLimit{
	...
	"packages/coding-agent/core/tools": {MaxFiles: 25, MaxLines: 2775, Reason: "ratcheted budget; one file per tool plus platform-split exec/replace files (bash_exec_unix/windows/other, file_replace_*), edit fuzzy NFKC/error-message + read/ls wording parity helpers, ShellEnv() PATH-injection helper (P1-G1), shared FileURLToPath parser (fileurl.go) reused by tools/path.go, NormalizePath, and the CLI file processor (P2-3 file:// unification), ripgrep-preferring grep with RE2 fallback (P2-2), and the detached-child PID registry/process-group liveness checks (detached.go + killProcessTreeByPID, P2-3); split if tool bodies grow"},
