...
func TestReadToolResolvesReadPathVariants(t *testing.T) {
	cwd := t.TempDir()
	path := filepath.Join(cwd, "Screen Shot 2026-05-28 at 10.00.00\u202fPM.txt")
	if err := os.WriteFile(path, []byte("image notes"), 0o644); err != nil {
		t.Fatal(err)
	}
	result := ReadTool{CWD: cwd}.Execute(context.Background(), raw(map[string]any{"path": "Screen Shot 2026-05-28 at 10.00.00 PM.txt"}), nil)
	if result.IsError || !strings.Contains(toolText(result.Content), "image notes") {
		t.Fatalf("read failed: %#v", result)
	}
}
