...
> **⚠️ Mostly reference for pi-go.** These are TypeScript/JavaScript extensions
> from the upstream Pi project. pi-go's minimal Node JSONL bridge supports
> extensions that register **custom tools**, subscribe to **event hooks**, and
> declare **CLI flags** (`registerFlag`/`getFlag`), but it does **not** yet
> implement the rest of the upstream `ExtensionAPI`: command dispatch, settings,
> a rich `ctx.ui`, message renderers, widgets, and provider/model registration.
> Any example below that relies on those (UI, renderers, providers, commands)
> will **not** run end-to-end on pi-go and is kept here as a reference for the
> API surface and as a porting target. See
> [`docs/EXTENSIONS_DESIGN.md`](../../docs/EXTENSIONS_DESIGN.md) for the runtime
> design and [`docs/TS_COMPATIBILITY.md`](../../docs/TS_COMPATIBILITY.md) for
> current gaps.
>
> The richer `--preset`/`ctx.ui`/command usage shown below applies to the
> upstream TypeScript runtime; on pi-go only the simple-tool subset works today.

## Usage (upstream TypeScript runtime)
